...
<#--<#macro "drop-down-onchange">
     ...
     <#if !allowMultiple>
         &lt;#&ndash; don't add first-in-list or empty option if allowMultiple (can deselect all to be empty, including empty option allows selection of empty which isn't the point) &ndash;&gt;
     ...
     &lt;#&ndash; <span>[${currentValue}]; <#list currentValueList as curValue>[${curValue!''}], </#list></span> &ndash;&gt;
     ...
     <#if isDynamicOptions>
         ...
         <script>
             function populate_${id}() {
                 ...
                 if (!hasAllParms) { $("#${id}").select2("destroy"); $('#${id}').html(""); $("#${id}").select2({ ${select2DefaultOptions} }); &lt;#&ndash; alert("not has all parms"); &ndash;&gt; return; }
            ...
            <#if isSavedFinds>
                ...
                <#if userFindInfoList?has_content>
                    ...
                    <select id="${quickSavedFindId}">
                        <option></option>&lt;#&ndash; empty option for placeholder &ndash;&gt;
                ...
                <#if (curPageMaxIndex > 4)>
                    ...
                    <script>
                        $("#${formId}_GoPage").validate({ errorClass: 'help-block', errorElement: 'span',
                            ...
                            &lt;#&ndash; show 1-based index to user but server expects 0-based index &ndash;&gt;
